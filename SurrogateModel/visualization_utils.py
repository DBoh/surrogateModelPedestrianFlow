...
class ExperimentVisualizer:
    ...
    def create_training_summary(self, history, config=None, zoomed=False):
        ...
        if 'out_rho_loss' in history.history:
            ...
            if zoomed:
                ...
                val_rho_loss = history.history['val_out_rho_loss'][start_epoch:] if 'val_out_rho_loss' in history.history else None
            else:
            ...
            if zoomed:
                ...
                val_v_loss = history.history['val_out_v_loss'][start_epoch:] if 'val_out_v_loss' in history.history else None
            else:
    ...
    def create_prediction_summary(self, predictions, labels, timesteps_to_show=None, metric_type="rmse"):
        ...
        if metric_type == "rmse":
            plt.suptitle('Prediction RMSE Visualization (Prediction - Ground Truth)\nBlue: Underestimation | Red: Overestimation', fontsize=12)
        else:
            plt.suptitle('Prediction Relative Error Visualization\nBlue: Underestimation | Red: Overestimation (±100%)', fontsize=12)
...
def create_experiment_summary(experiment_path, history=None, dataset=None, config=None):
    # alle plots auf einmal machen
    visualizer = ExperimentVisualizer(experiment_path)
    
    # Config JSON im Hauptordner speichern
    ...
    # Training Summary nur einmal im Hauptordner (ist für beide Metriken gleich)
    if history:
        # Normale Version mit vollem Bereich
        ...
        # Gezoomte Version ohne initialen Spike
    ...
    # Metriken speichern (nur einmal im Hauptordner)
    if history:
